Public Class EJERCICIO_PROPIAS_4

Private Sub EJERCICIO_PROPIAS_4_Load(sender As Object, e As EventArgs) Handles MyBase.Load
   Dim numero_random As New Random
    ...
    Dim numerador_3 As Integer = numero_random.Next(1, 30)
    Dim divisor_3 As Integer = numero_random.Next(10, 30)
'>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
Do While (numerador_1 >= divisor_1 Or numerador_2 >= divisor_2 Or numerador_3 >= divisor_3)
    ...
    If (numerador_3 >= divisor_3) Then
        numerador_3 = numero_random.Next(1, 30)
    End If
Loop
...
Private Sub btn_verificar_Click(sender As Object, e As EventArgs)
